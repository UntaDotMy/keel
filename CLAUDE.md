...
**OpenCode host (bridge)**: Claude Code's lifecycle hooks do not fire in OpenCode, so the automatic behaviors (skill auto-load, session-start context push, observation capture, session-end learning, post-compact re-push) are delivered through an OpenCode plugin instead. `claude-skills install` copies `opencode/claude-core.ts` into `~/.config/opencode/plugins/` (the plural directory OpenCode loads) and merges a `claude_core` MCP entry into `~/.config/opencode/opencode.json` (merge, never clobber; BOM-tolerant). The plugin maps OpenCode events to a host-neutral CLI surface, `claude-skills bridge <event>`: `session-start` and `post-compact` print the bootstrap/digest/post-compact context; `user-prompt` prints the composite per-prompt context (skill brief + iron law + pointers) in one call; `observe` records a tool observation from stdin JSON; `session-end` runs the learning cycle + session summary capture; `gate-status` prints fired/cleared gates. Every `bridge` subcommand prints plain text and always exits 0 (never blocks a turn). The plugin injects via `chat.message` (per-prompt + once-per-session start, marker-guarded) and `experimental.session.compacting`, records observations on the `tool.execute.after` event (fire-and-forget), and triggers learning on `session.compacted` / session-end on `session.deleted`. It never throws and caps every bridge call at a 500ms timeout, degrading to "no context injected" on failure. The bridge reuses the same Rust handlers as the Claude Code hook path — one source of truth, no logic fork.
