...
**Hook events** (`.claude/hooks.json`) are wired through `keel hook <event>` for every harness lifecycle event the manager observes. The `HOOK_EVENTS` table in `rust/crates/keel/src/hooks/claude.rs` is the single source of truth. It defines every event, its slug/matcher, and whether it auto-installs into `settings.json`. Most rows install automatically. Fourteen rows carry installs_in_settings: false (pinned by only_known_events_opt_out_of_install): reserved no-op events (TaskCreated, TaskCompleted, TeammateIdle, WorktreeCreate, WorktreeRemove, Setup, InstructionsLoaded, ConfigChange, Elicitation, ElicitationResult, PreModelSwitch, PostModelSwitch) plus structural opt-outs (FileChanged: matcher is the watch list so empty matcher ships dead config; MessageDisplay: would silently rewrite on-screen text). All 33 official events remain dispatchable via keel hook <slug>. Both still dispatch for ad-hoc invocations (`keel hook file-changed`, `keel hook message-display`). Actively implemented handlers: `SessionStart` (MCP self-heal + bootstrap context + watchPaths), `UserPromptSubmit` (skill routing + compression nudge), `PreToolUse` (command rewrite + auto-allow rules), `PostToolUse` (timings + observations + comment-style lint on Edit/Write + system map refresh, runs async; the comment lint is advisory, env-gated by `CLAUDE_SKILLS_COMMENT_LINT_GATE`, with `off` disabling it, and catches over-length/chatty/first-person/em-dash comments at write time so they don't wait for review), `PostToolUseFailure` (timings + failure observation, runs async), `PostToolBatch` (review/brief/closeout gates), `Stop` (bounded closeout guard using the official top-level `decision: "block"` contract; it honors `stop_hook_active`, never emits `additionalContext`, and allows after the cap or any state error), `SubagentStart` (iron law context injection), `SubagentStop` (lifecycle), `PermissionRequest` (auto-approve keel commands), `PermissionDenied` (retry signaling), `CwdChanged` (system map refresh), `DirectoryAdded` (system map refresh for `/add-dir`), `SessionEnd` (auto-capture + learning + prune), `PreCompact`/`PostCompact` (learning + system map), `Notification` (terminal bell). Remaining events dispatch through the lifecycle wildcard for forward-compatibility. Hook types supported: `command` (shell scripts), `http` (POST event JSON to a URL), `mcp_tool` (call an MCP server tool), `prompt` (evaluate a prompt with an LLM using `$ARGUMENTS` for context), and `agent` (run an agentic verifier with tools for complex verification). Elicitation events (`Elicitation`, `ElicitationResult`) handle MCP server user-input requests. The hook can accept/decline/cancel via `hookSpecificOutput.action`. When Anthropic adds or renames events, update both `HOOK_EVENTS` and the generated `.claude/hooks.json`. Reference: https://code.claude.com/docs/en/hooks.
...
The numbered operating contract (rules 0-9) lives once, in [`AGENTS.md`](AGENTS.md) § Core Operating Contract. Claude Code does not read AGENTS.md natively, so it is imported here per the official memory-import mechanism:

@AGENTS.md

Claude-Code-specific supplements (unique to this host adapter; numbering follows the original block):
...
The enforcement gates are the model-independent backstop for the Iron Law. Hooks cannot force a tool/Skill call, but they can inject a reminder when a required artifact is missing. Most gates fire on PostToolBatch (mid-turn checkpoint); each is controlled by its own `CLAUDE_SKILLS_*_GATE` env var. The `Stop` handler is a bounded closeout backstop: it honors `stop_hook_active`, reads the same gate state, emits only the official top-level `decision: "block"` response while under its three-block cap, and allows on state read/write errors so a damaged counter cannot wedge the session. `SubagentStop` remains lifecycle-only. All gate implementations live in `rust/crates/keel/src/runner/hook_lifecycle/mod.rs` (`run_hook_post_tool_batch`, ~`:3227`):
...
Why most gates ride PostToolBatch: PostToolBatch fires after every tool batch and can nudge mid-turn without forcing an extra turn. Stop is reserved for the bounded end-of-turn decision guard; it does not inject `additionalContext`, and it honors `stop_hook_active` to avoid a continuation loop. `SessionEnd` carries no `hookSpecificOutput` at all. PostToolBatch is the *primary* checkpoint because it fires most often; Stop is the final closeout backstop.
