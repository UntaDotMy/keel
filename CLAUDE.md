...
- 24 specialist the harness **skills** for software delivery (`<name>/SKILL.md`)
...
- 24 matching the harness **subagents** for token-efficient delegation (`.claude/agents/<name>.md`)
...
- `.claude/hooks.json` — the harness hook wiring rendered by `keel hook install`.
- `.claude-plugin/plugin.json` — Plugin manifest for the harness's plugin system.
