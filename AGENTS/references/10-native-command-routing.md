...
- **Level 3: Hook guidance:** `keel hook install` writes the managed lifecycle set to `~/.claude/settings.json`; use `keel hook list` for the authoritative installed events. `PreToolUse` owns token-saving interception because it must run before noisy shell output exists. The hook may return `permissionDecision: "allow"` with a `toolInputOverride` that transparently wraps the command (not a block-and-rerun).
