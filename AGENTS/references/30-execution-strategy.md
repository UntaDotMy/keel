...
- **Never assume, never guess, never skip or shortcut.** Fetch the owning source and the current official contract. Do not skip a required loop step, test, review, sibling scan, or schema check. A one-site fix of a class bug is unfinished.
...
- **Native keel Invocation Rule:** Treat `keel ...` as the command shape. `keel install` persists the native binary directory on the user's PATH for new sessions; an already-open shell may need to be restarted. From a source checkout, run `cargo run --bin keel -- ...` or the built executable directly. When PATH is unavailable, use the explicit native binary such as `~/.keel/keel` or `%USERPROFILE%\\.keel\\keel.exe`. Do not use shell or PowerShell wrapper launchers as a substitute for the native executable.
