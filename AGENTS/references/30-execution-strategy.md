...
- If that scoped `SYSTEM_MAP.md` is missing, stale, or contradicted by the current code, refresh it first with `keel memory system-map refresh` and keep it in Claude Code-global project-scoped storage instead of the user workspace.
...
- Prefer the installed Claude Code-home root executable when the skill pack is already synced: `~/.claude/keel` on macOS or Linux and `~/.claude/keel.exe` on Windows are the canonical direct-run paths for the managed install and the preferred global discovery paths for agents.
...
- **Implemented vs Planned Memory Commands (read before following the rules below):** The implemented memory/orchestration surface under both `memory` and `memoriesv2` now covers: `scope resolve`, `system-map refresh`, `working-brief`, `completion-gate check`, `recall` (FTS5 search), `research-cache` (record/lookup/stale/reward/list), `maintenance` (append-working-buffer/trim/recalibrate), `agent-registry` (register/list), `agent-packets` (build/show/list), `loop-guard` (record/check), `entity` (upsert/list/query), `graph` (add/list/query), `retrieve` (cross-family lexical search), `instincts`, and `status`. `memory report` (an alias for `status`) and `memory index` (rebuilds the FTS5 recall index) are also implemented. The workflow ledger (`workflow route|start|cockpit|finish|resume`) and `orchestration` (`runtime-preflight|resume-status|task begin|progress|complete|list|checkpoint`) are also implemented. `memory hook` is intentionally NOT a memory subcommand — it exits with a pointer to `keel hook install|list|instructions|diagnose`, which owns Claude Code lifecycle hooks. Still **planned and currently exit non-zero with a "not implemented" message**: `memory working-brief record-summary`, `memory completion-gate record-requirement`, and `consolidate`. Where a rule names a planned command, satisfy the intent through the implemented surface plus plain L1 files (`SESSION-STATE.md`, `working-buffer.md`). Do not treat a planned command's "not implemented" exit as a task failure.
- **Second-Layer Memory Rule:** The supported write surface stays the Rust-native `keel memory ...` commands, but those writes must keep `memoriesv2` synchronized as the global second-layer memory instead of leaving durable workflow state only in chat or only in first-layer Claude Code memory. Use `keel memoriesv2 scope resolve` to inspect the resolved second-layer files before broad memory reads or when validating where durable state landed.
