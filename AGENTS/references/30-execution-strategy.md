...
- **Native keel Invocation Rule:** Treat `keel ...` as the command shape, not a guaranteed literal executable name. The managed install keeps the native binary in the harness home root; it does not make bare `keel ...` globally available by default. From a source checkout, run `cargo run --bin keel -- ...`. After install, run the explicit native binary such as `~/.claude/keel ...`, `%USERPROFILE%\\.claude\\keel.exe ...`, `./keel ...`, or `.\\keel.exe ...`. Do not use shell or PowerShell wrapper launchers.
...
- **Layered Memory (one CLI, layered paths):** Keep memory layered the way a human would, all under the unified layout. Harness built-in Auto memory is the incidental first layer. Repo-owned durable layers live under `~/.claude/memories/` (workspace, workstream, optional lane, reference) plus family records and working-briefs. Use high-level reusable guidance in summaries, workspace-scoped notes under `~/.claude/memories/workspaces/<workspace-slug>/`, workstream notes under `.../workstreams/<workstream-key>/`, optional lane notes under `.../lanes/<agent-instance>/`, research-cache findings with freshness metadata, and exact commands/errors only in deeper task notes when reusable.
- **L1 L2 L3 Memory Map:** Treat the small always-read workspace guidance, summaries, `SESSION-STATE.md`, and `working-buffer.md` as L1 brain files; keep scoped `~/.claude/memories/` workspace, workstream, and lane material as L2 working memory; keep deeper SOPs, playbooks, and scoped `reference/` material as L3 opened on demand. One home per fact; information should flow down instead of being duplicated across every layer.
- **Implemented Memory Commands (read before following the rules below):** There is one unified CLI group: `keel memory` (the old `memoriesv2` command group was removed in #150). It covers: `scope resolve`, `system-map refresh`, `working-brief`, `completion-gate check`, `recall` (FTS5 search), `research-cache` (record/lookup/stale/reward/list), `maintenance` (append-working-buffer/trim/recalibrate), `agent-registry` (register/list), `agent-packets` (build/show/list), `loop-guard` (record/check), `entity` (upsert/list/query), `graph` (add/list/query), `retrieve` (cross-family lexical search), `instincts`, and `status`. `memory report` (alias for `status`) and `memory index` (rebuilds the FTS5 recall index) are also implemented. The workflow ledger (`workflow route|start|cockpit|finish|resume`) and `orchestration` (`runtime-preflight|resume-status|task begin|progress|complete|list|checkpoint`) are also implemented. `memory hook` is intentionally NOT a memory subcommand. It exits with a pointer to `keel hook install|list|instructions|diagnose`. Also implemented: `memory working-brief record-summary`, `memory completion-gate record-requirement`, and `memory consolidate` (family-directory status scan with counts/previews; not a merge/promote). Use `keel memory consolidate` for a structured family snapshot; use the `memory-consolidation` skill when you need prose notes written into durable memory.
- **Unified Memory Rule:** Write durable state through `keel memory ...` only. Workspace, workstream, role, and family layers live under the single memory layout (`~/.claude/memories/…` and related lanes), not a second CLI group. Use `keel memory scope resolve` before broad memory reads when validating where durable state landed.
...
- **Resume Status Rule:** At the start of each non-trivial turn, run `keel orchestration resume-status`. Do not wait to "notice" compaction from the transcript; reconstruct the active workstream from durable artifacts first.
- **Task Lifecycle Rule:** After `resume-status` and before meaningful work, capture the live task with `keel orchestration task begin --task "<description>" [--phase <phase>] [--next-step <text>]`. As work advances, run `keel orchestration task progress --id <id> [--note <text>]`, and `keel orchestration task complete --id <id>` when done; `orchestration task list [--open-only]` shows the live set. Keep the scoped working brief current alongside it so requirement state and plan items do not drift in chat-only prose.
- **Runtime Preflight Rule:** When the runtime tool surface or unified-exec health is uncertain, run `keel orchestration runtime-preflight` first and follow its preferred tool routing before opening new long-running work.
- **Compaction Recovery Rule:** Before manual compaction, and whenever automatic compaction risk is rising, run `keel orchestration checkpoint --note "<where you are>"` to persist a snapshot (open tasks, open workflows, working-brief count) and surface the current durable state. After compaction, resets, or `resume-status` continuity warnings, reload the scoped artifacts before resuming implementation, then use `keel code-search search` to reacquire the exact code surface instead of replaying the whole transcript from memory.
