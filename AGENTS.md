...
11. **Mandatory external research (every fix, trace, or implementation).** Before fixing, tracing, or implementing, do a live web lookup (WebSearch/WebFetch/context7; Stack Overflow or vendor docs as fitting). Model memory is stale by cutoff. `recall`, memory, and host reads are a hypothesis, never proof. The requirement is **per problem**, not per session: a new problem needs new evidence; reuse happens only through a fresh `research-cache` entry. An unresearched fix or trace is a guess. See [30-execution-strategy.md](AGENTS/references/30-execution-strategy.md) § 1.
12. **Implementation law (7 steps, every code change).** (1) Read the target file and the exact lines before patching, then web search/research. (2) Trace the function and full call chain; reason about the effect. (3) Preserve existing flow, prefer the shared/class/owner file over hardcoding the same behavior across files, and research the language best practice. (4) Implement with the AI-slop comment cap (max 2 lines) enforced **at write time**, not at review. (5) Re-read and re-trace the fix. (6) Loop and review. (7) Confirm every requirement against evidence, then produce the final output. See [30-execution-strategy.md](AGENTS/references/30-execution-strategy.md) § 4.
