...
2. **Native commands before raw shell.** Prefer `keel anvil`, `keel run -- <command>`, `keel code-search search`, `keel flow ...`, and `keel review ...` when those surfaces own the job. See [10-native-command-routing.md](AGENTS/references/10-native-command-routing.md).
...
4. **Iterative loop.** ALIGN → RESEARCH → PLAN → IMPLEMENT → TEST → FIX → VERIFY → REVIEW → RECONCILE. For ambitious or multi-piece work, run `keel anvil` (compile→cast→sieve→stamp→loop, `running-anvil` skill). See [30-execution-strategy.md](AGENTS/references/30-execution-strategy.md).
