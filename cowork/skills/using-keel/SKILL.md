...
5. **Research externally, per problem.** Every fix, trace, or implementation starts with a live web lookup (WebSearch/WebFetch/context7) for that problem; model memory is stale by cutoff, and `recall`/memory are a hypothesis, never proof. Reuse a fresh `research-cache` entry instead of re-browsing. The 7-step implementation law is in `AGENTS/references/30-execution-strategy.md` § 4.
...
4. After implementation: run `keel code-search siblings` and handle every hit, then build/test/lint this turn. Non-trivial changes → reviewer / `keel review pre-pr`. Surface extra defects found; ask how to handle them.

**Enforcement (default-on):** PreToolUse **denies** edit-class tools until **fresh external research** ran: a WebSearch/WebFetch/context7 lookup, or a fresh `keel memory research-cache` record/reward (reuse). `recall` / `system_map` / reads alone do **not** clear VERIFIED, which is the default. Opt down with `KEEL_IRON_LAW_GATE=strict` (any keel tool), `=balanced`, or `=off`. Iron law re-injects at SessionStart, every prompt, PostCompact, and SubagentStart; it never drops mid-work. PostToolBatch brief/review/completeness gates default to hard feed-forward until a working brief, sibling scan (`code-search siblings`), and reviewer pass exist. **Research enforcement (per problem):** `research-enforcement` skill: reuse gate → R1 authoritative live web (≥1 live pass for every problem) → R2 community/issues → R3 broad; loop-back with refined terms until specific; never trust stale memory, never prompt the user for verifiable facts, never accept a generic answer. On error, websearch the exact error text and retry with refined prompts before patching. Anvil verification: `keel anvil sieve` (0-LLM gates) + `keel anvil stamp` (evidence rank) then bounded `keel anvil loop` only if gates fail. Opt-down via env (`KEEL_IRON_LAW_GATE`, `CLAUDE_SKILLS_*_GATE`); never invent scope to "clear" a gate.
...
`anvil` is the only delivery loop. On every host call the `anvil` MCP tool (`compile`, then `run` with `--dry-run`) before hand-editing. The host Skill catalog may omit `running-anvil` — that is not a reason to skip Anvil; the MCP tool is the path. `loop`, live `run`, `git-workflow await-ci`, and `review pre-commit`/`pre-pr` start in the background on MCP (poll `command_output`; stop with `command_kill`) so they cannot hang the host's tool slot. Bank: `<keel-home>/memories/workspaces/<slug>/anvil/`. Sprint/user-story/workflow surfaces are deleted.

**Completeness (class, not one hit):** after any fix or implement, run `keel code-search siblings` (MCP `code_search` `action=siblings`) and handle every similar, related, leftover, host/CLI/test/install copy in this turn. Anvil sieve `echo ok` is not that scan. PostToolBatch completeness and `keel review` fail closed until the scan ran. A one-site change is unfinished.
