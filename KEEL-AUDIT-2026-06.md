...
| **Orchestration** | | | | |
| Real git worktree dispatch | ✅ `keel dispatch`, fail-closed merge (stale: not shipped — no `keel dispatch` command exists) | ❌ | ❌ (markdown only) | ❌ |
| Git-backed code checkpoints | ✅ `keel checkpoint` (stale: not shipped — removed before release) | ❌ | ❌ | ❌ |
...
8. **Dark Factory remote agents** — SAW's tmux-based remote agents are useful but outside keel's single-binary stance. The dispatch system (`keel dispatch`) handles parallel worktree isolation locally. (Stale: not shipped — no `keel dispatch` command exists.)
