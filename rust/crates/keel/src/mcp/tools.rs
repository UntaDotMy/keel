...
fn tools_list_catalog() -> Value {
    json!({
        "tools": [
            ...
            {
                "name": "decision",
                "description": "Jev-style typed decisions with calibrated confidence: score (review/plan rubrics), noul (shell danger probability), choice (skill composition), calibrate (routing confidence), conformal (conformal risk control), review-feedback (accuracy outcomes), noul-feedback (shell override labels), cache-stats (hit/miss counters), calibration-report (calibration health), samples (offline training corpus counts), benchmark (routing accuracy vs a remote zero-shot API). Deterministic and local; escalates below 0.6 confidence.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "action": { "type": "string", "description": "Decision operation: score, noul, choice, calibrate, conformal, review-feedback, noul-feedback, cache-stats, calibration-report, samples, benchmark." }
                    },
                    "required": ["action"]
                }
            }
...
fn encode_skill_catalog_cursor(
    ...
    let home_fingerprint =
        crate::utility::hashing::sha256_hex(claude_home.to_string_lossy().as_bytes());
    let claims = SkillCatalogCursorClaims {
        v: 1,
        snapshot: snapshot.chars().take(32).collect(),
        home: home_fingerprint[..16].to_string(),
...
fn decode_skill_catalog_cursor(
    ...
    let home_fingerprint =
        crate::utility::hashing::sha256_hex(claude_home.to_string_lossy().as_bytes());
    let expected_snapshot: String = snapshot.chars().take(32).collect();
    if claims.v != 1
        || claims.snapshot != expected_snapshot
        || claims.home != home_fingerprint[..16]
