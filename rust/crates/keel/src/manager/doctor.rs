...
fn probe_mcp_discover_with_timeout(
    ...
    let request = serde_json::json!({
        ...
        "params": {
            "_meta": {
                ...
                "io.modelcontextprotocol/clientInfo": { "name": "doctor", "version": "1.0" }
            }
        }
    });
    if let Some(mut stdin) = child.stdin.take() {
    ...
    let responded = match receiver.recv_timeout(timeout) {
        Ok(response) => valid_mcp_discovery_response(&response),
...
fn valid_mcp_discovery_response(response: &str) -> bool {
    response.lines().any(|line| {
        ...
        value["jsonrpc"] == "2.0"
            ...
            && value["result"]["capabilities"].is_object()
...
mod tests {
    ...
    #[test]
    fn mcp_probe_accepts_only_correlated_modern_discovery() {
        let modern = serde_json::json!({"jsonrpc":"2.0","id":1,"result":{"resultType":"complete","supportedVersions":["2026-07-28"],"capabilities":{"tools":{}}}});
        assert!(valid_mcp_discovery_response(&modern.to_string()));
        let mut wrong_id = modern.clone();
        wrong_id["id"] = serde_json::json!(2);
        assert!(!valid_mcp_discovery_response(&wrong_id.to_string()));
        let mut wrong_version = modern;
        wrong_version["result"]["supportedVersions"] = serde_json::json!(["2025-11-25"]);
        assert!(!valid_mcp_discovery_response(&wrong_version.to_string()));
        assert!(!valid_mcp_discovery_response(
            r#"{"jsonrpc":"2.0","id":1,"result":{"protocolVersion":"2025-11-25"}}"#
        ));
        assert!(!valid_mcp_discovery_response(
            r#"{"jsonrpc":"2.0","id":1,"error":{"code":-32022,"message":"unsupported protocolVersion"}}"#
        ));
    }
