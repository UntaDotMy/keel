...
6. **Invoke relevant skills.** When a keel skill plausibly matches, check its trigger before loading it; skip keyword-only matches that do not fit the task.
...
11. **Mandatory external research.** Every fix, trace, or implementation starts with a live web lookup (WebSearch/WebFetch/context7) for that problem; model memory is stale by cutoff. `recall` and memory are a hypothesis, never proof. Do not fix or trace blind; reuse a fresh research-cache entry instead of re-browsing the same problem.
