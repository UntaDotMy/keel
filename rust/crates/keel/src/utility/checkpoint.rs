...
fn render_help(standard_output: &mut dyn Write) {
    let _ = writeln!(
        standard_output,
        "Usage: keel checkpoint [create|list|show|restore] [flags]"
    );
    let _ = writeln!(
        standard_output,
...
fn run_create(
    ...
    match create_checkpoint(&root, &label, standard_error) {
        Some(id) => {
            ...
            let _ = writeln!(
                standard_output,
                "  restore with: keel checkpoint restore --id {id} --confirm"
            );
            0
        }
        None => 1,
...
fn run_list(
    ...
    if entries.is_empty() {
        let _ = writeln!(
            standard_output,
            "  none yet — create one with: keel checkpoint create"
        );
    }
    0
}
...
fn run_restore(
    ...
    match safety_id {
        Some(safety) => {
            let _ = writeln!(
                standard_output,
                "  pre-restore safety snapshot: {safety} (undo this restore with: keel checkpoint restore --id {safety} --confirm)"
            );
        }
        None => {
            let _ = writeln!(
...
mod tests {
    ...
    fn unique_repo(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!("keel-ckpt-{label}-{pid}-{unique}"))
    }
