...
/// CLI: `keel dispatch <plan|start|status|complete|merge|abandon|list> [flags]`.
...
fn render_help(standard_output: &mut dyn Write) {
    let _ = writeln!(
        standard_output,
        "Usage: keel dispatch <plan|start|status|complete|merge|abandon|list> [flags]\n\
         \n\
...
fn run_plan(
    ...
    if flags.bool_value("json") {
        ...
        let _ = writeln!(standard_output, "  next: keel dispatch start --id {id}");
        0
    }
}
...
fn run_start(
    ...
    if flags.bool_value("json") {
        ...
        let _ = writeln!(
            standard_output,
            "  work + commit in that worktree, then: keel dispatch complete --id {id}"
        );
        0
    }
}
...
fn run_complete(
    ...
    if flags.bool_value("json") {
        ...
        let _ = writeln!(
            standard_output,
            "  merge with: keel dispatch merge --id {id} --confirm"
        );
        0
    }
}
...
fn run_merge(
    ...
    if flags.bool_value("json") {
        ...
        let _ = writeln!(
            standard_output,
            "  reclaim its worktree with: keel dispatch abandon --id {id} --confirm"
        );
        0
    }
}
