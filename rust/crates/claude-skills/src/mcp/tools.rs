//! Purpose: `tools/list` schema and `tools/call` dispatch for the claude_core
...
pub(super) fn handle_tools_list() -> Value {
    json!({
        "tools": [
            ...
            {
                "name": "skill_get",
                "description": "Load the full SKILL.md body for an installed claude_core skill by name. Use after skill_route (or skill_list) when you need the complete skill, not just the brief. Returns the entire file including frontmatter.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
            ...
            {
                "name": "skill_list",
                "description": "List every installed claude_core skill with its name, description, and when_to_use. Use to discover what skills exist before routing or loading one.",
                "inputSchema": {
                    "type": "object",
                    "properties": {}
                }
...
fn tool_system_map(arguments: &Value) -> Result<String, String> {
    ...
    // the backing for the claude_core://system-map resource, which should keep
