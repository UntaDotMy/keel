...
/// CLI dispatcher for `claude-skills telemetry`. Mirrors the `flow` shape
...
fn render_telemetry_help(standard_output: &mut dyn Write) {
    let _ = writeln!(
        standard_output,
        "Usage: claude-skills telemetry [summary] [flags]"
    );
    let _ = writeln!(
        standard_output,
...
mod tests {
    ...
    fn with_isolated_claude_home<F: FnOnce(&PathBuf) -> R, R>(suffix: &str, run: F) -> R {
        ...
        let root = std::env::temp_dir().join(format!(
            "claude-skills-telemetry-{}-{nanos}-{suffix}",
            std::process::id()
        ));
    ...
    fn run_telemetry_command_renders_help_when_no_subcommand() {
        ...
        assert_eq!(exit, 0);
        assert!(String::from_utf8_lossy(&stdout).contains("Usage: claude-skills telemetry"));
        assert!(stderr.is_empty());
    }
