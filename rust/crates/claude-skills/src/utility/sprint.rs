//! Purpose: Scrum-style sprint loop ledger, backing `claude-skills sprint`.
...
/// CLI: `claude-skills sprint <plan|status|advance|review|list> [flags]`.
pub fn run_sprint_command(
    ...
    if action.is_empty() || matches!(action, "help" | "--help" | "-h") {
        let _ = writeln!(
            standard_output,
            "Usage: claude-skills sprint <plan|status|advance|review|list> [flags]\n\
             \n\
...
fn run_review(
    ...
    if complete {
        ...
        let _ = writeln!(
            standard_output,
            "  Next: demo the increment and capture a retro (claude-skills memory ...)."
        );
        0
    } else {
        let _ = writeln!(
...
mod tests {
    ...
    fn isolated<F: FnOnce(&str)>(label: &str, body: F) {
        ...
        let root: PathBuf = std::env::temp_dir().join(format!(
            "claude-skills-sprint-{}-{nanos}-{label}",
            std::process::id()
        ));
