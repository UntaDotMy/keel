...
impl Observation {
    fn to_json(&self) -> Value {
        ...
        Value::Object(vec![
            ...
            (
                "tokenAnalytics".into(),
                Value::String(
                    "see `claude-skills gain` and `claude-skills session` for the token-savings axis"
                        .into(),
                ),
            ),
        ])
    ...
    fn render_text(&self, standard_output: &mut dyn Write) {
        let _ = writeln!(standard_output, "claude-skills observe: session health");
        ...
        let _ = writeln!(
            standard_output,
            "tokens: run `claude-skills gain` / `claude-skills session` for the savings axis"
        );
    }
}
...
mod tests {
    ...
    fn observe_json_reports_all_axes_for_empty_home() {
        ...
        let home = tempdir_under("claude-skills-observe-empty").join("home");
    ...
    fn observe_text_surface_names_all_axes_and_points_to_gain() {
        let _guard = ENV_LOCK
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        let home = tempdir_under("claude-skills-observe-text").join("home");
        fs::create_dir_all(&home).expect("create home");
        ...
        assert!(
            rendered.contains("claude-skills gain"),
