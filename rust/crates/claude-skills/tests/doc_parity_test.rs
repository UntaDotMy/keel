...
//!   flagged as drift (CLAUDE.md/using-claude-core claimed "43 directories" and
...
/// Repo root: the claude-skills crate lives at `rust/crates/claude-skills`, so
...
/// which holds a benchmark-artifact SKILL.md that is not a claude-core skill.
...
/// Every first-party skill directory (except the `using-claude-core` bootstrap,
...
fn every_first_party_skill_is_in_manifest() {
    ...
    let not_in_manifest: Vec<&String> = on_disk
        .iter()
        .filter(|name| name.as_str() != "using-claude-core" && !manifest.contains(*name))
        .collect();
    assert!(
...
/// added or removed, this assertion fails until CLAUDE.md / using-claude-core
...
fn documented_skill_counts_match_disk_and_manifest() {
    ...
    assert_eq!(
        ...
        "expected 43 manifest skills (24 specialists + 18 technique + requesting-code-review); \
         got {}. If this changed intentionally, update CLAUDE.md and using-claude-core/SKILL.md.",
        manifest.len()
    );
    assert_eq!(
        on_disk.len(),
        44,
        "expected 44 first-party SKILL.md dirs (43 manifest skills + using-claude-core bootstrap); \
         got {}. Update the docs and this test together.",
        on_disk.len()
    );
    assert!(
        on_disk.contains("using-claude-core"),
        "using-claude-core bootstrap directory must exist"
    );
    assert!(
        !manifest.iter().any(|name| name == "using-claude-core"),
