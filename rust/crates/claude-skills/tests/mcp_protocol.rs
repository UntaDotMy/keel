//! Purpose: End-to-end harness that spawns the `claude-skills` binary in
...
//! Caller: `cargo test -p claude-skills --test mcp_protocol`.
//! Dependencies: serde_json for request/response framing, the cargo-injected
//!   `CARGO_BIN_EXE_claude-skills` env var for the binary under test, the
...
const BINARY_ENV_VAR: &str = "CARGO_BIN_EXE_claude-skills";
...
impl McpServerProcess {
    fn spawn(claude_home: &Path) -> Self {
        ...
        let mut child = command.spawn().expect("spawn claude-skills mcp serve");
    ...
    fn close(mut self) {
        ...
        assert!(
            status.success(),
            "claude-skills mcp serve exited with status {status:?}"
        );
    }
}
...
fn unique_temp_directory(label: &str) -> PathBuf {
    let unique_suffix: u128 = SystemTime::now()
        ...
        .unwrap_or_default();
    let candidate = env::temp_dir().join(format!("claude-skills-mcp-{label}-{unique_suffix}"));
...
fn mcp_serve_resources_list_includes_system_map_and_recall_status() {
    ...
    let uris: Vec<String> = response["result"]["resources"]
        ...
        .collect();
    assert!(uris.contains(&"claude_core://system-map".to_string()));
    assert!(uris.contains(&"claude_core://recall/status".to_string()));
