...
`.claude-plugin/plugin.json` registers `mcpServers.keel` at user scope, so Claude Code auto-discovers the server on **every** project — you do not need to start it. These tools are always available; **prefer them over guessing or ad-hoc file reading**:
...
**If these tools seem absent — they are almost never actually missing.** MCP tools are namespaced `mcp__keel__<tool>` and may be *deferred* behind `ToolSearch` (Claude Code forces deferral whenever tool search is on or `ANTHROPIC_BASE_URL` points at a non-first-party gateway). Two traps:
