...
`anvil` is the only delivery loop. On every host call the `anvil` MCP tool (`compile`, then `run` with `--dry-run`) before hand-editing. The host Skill catalog may omit `running-anvil` — that is not a reason to skip Anvil; the MCP tool is the path. `loop`, live `run`, `git-workflow await-ci`, and `review pre-commit`/`pre-pr` stay on the CLI because they exceed the ~30s MCP host budget — the `cli` and sync `run_command` passthroughs refuse them too. Bank: `<keel-home>/memories/workspaces/<slug>/anvil/`. Sprint/user-story/workflow surfaces are deleted.
