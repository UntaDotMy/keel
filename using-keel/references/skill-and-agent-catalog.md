...
- `running-anvil` — Only delivery loop: `compile` (`--goal/--bar/--files` → `anvil.lock.json` + prefix + gates) → `cast` (N isolated workspaces, frozen prefix, host CLI builder) → `sieve` (0-LLM gates) → `stamp` (evidence rank: passing gate, then smaller clipped output; `--strict` fail-closed) → `loop` (bounded refinement, only if gates fail, max 20, delta 0.05, 300s wall, gate pass fraction) → `anvil.report.json` with metrics every run. Bank: `<keel-home>/memories/workspaces/<slug>/anvil/`. Backed by `keel anvil`.
...
- `writing-skills` — Author and revise skills with evidence the prose changes behavior: RED-GREEN-REFACTOR on the instructions themselves, pressure-testing a fresh isolated evaluator without the skill, then with it. Use subagents only when host policy allows them; otherwise use the native evaluator or a disposable session. The behavioral gate above skill-lint's structural gate.
