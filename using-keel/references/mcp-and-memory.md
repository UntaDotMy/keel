...
- Tool `skill_get` (full name `mcp__keel__skill_get`) — load the full SKILL.md body for an installed skill by name. The full-body upgrade after `skill_route`. **Also the fallback when the host Skill tool returns `Unknown skill`:** if `Skill("authentication-and-identity")` (or any catalog name) fails, call `skill_get` with that exact name — the file is installed under `~/.claude/skills/<name>/` even when the host registry is stale or incomplete.
...
- Tool `cli` (full name `mcp__keel__cli`) runs other implemented keel subcommands (`review`, `git-workflow`, `memory`, `flow`, `code-search`, `config-audit`, `skill-lint`, `gain`, `telemetry`, `status`, `doctor`, and related surfaces) and returns compacted output. Pass `args` as a string array. Destructive/management subcommands require `confirm: true`; `mcp` is refused. Prefer a dedicated tool when one fits; use `cli` for the rest.
