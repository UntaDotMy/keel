...
- Tool `cli` (full name `mcp__keel__cli`) — run other implemented keel subcommands (`review`, `git-workflow`, `memory`, `flow`, `code-search`, `config-audit`, `skill-lint`, `gain`, `telemetry`, `status`, `doctor`, and related surfaces) and get compacted output. Pass `args` as a string array. Destructive/management subcommands require `confirm: true`; `mcp` is refused. Prefer a dedicated tool when one fits; use `cli` for the rest.
