...
0. **Understand before building** — before writing any code, restate what the request actually asks, confirm the user story, and research what is genuinely needed. No guessing, no assuming, no building against an imagined spec. Correct code that solved the wrong problem still gets thrown away, so this gates every rule below: there is no point routing a skill or refreshing memory for the wrong task. If the request is ambiguous in a way that changes what you build, ask before building, not after. **For any requirement-bearing prompt** (a feature, change, fix, or behavior ask — not just "build X"), the confirmation in this rule is done through `writing-user-stories`: convert the prompt *completely* into strict Agile/Jira user stories (Connextra "As a `<role>`, I want `<goal>`, so that `<benefit>`" + Gherkin Given/When/Then acceptance criteria, validated against INVEST), validate the format with `keel user-story lint`, then confirm correctness and completeness with the user via `AskUserQuestion` (do not end the turn) before building. The confirmed stories are captured in the working brief and are the anti-drift spec `reviewer` Stage 1 reconciles the diff against — every requirement maps to a story, and nothing is built that no story asked for. Pure questions, lookups, and already-confirmed trivial edits are exempt.
1. **Skills first** — route domain work through the matching `~/.claude/skills/<name>/SKILL.md`. On a requirement-bearing prompt, run `writing-user-stories` first to pin and confirm the requirement. Run `preserve-existing-flow` before editing existing source. Run `reviewer` before closing **non-trivial** work (logic changes, multi-file edits, public-API touches, security-sensitive surfaces, brownfield behavior changes, release-impacting work). Skip `reviewer` for trivial work: docs-only, formatting-only, generated-only, single-line typo or comment fixes, and explicitly throw-away work.
...
4. **Iterative loop** — ALIGN → RESEARCH → PLAN → IMPLEMENT → TEST → FIX → VERIFY → REVIEW → RECONCILE. For multi-story or multi-step builds, run this as a **Scrum-style sprint** (`running-a-sprint`): the confirmed user stories become the sprint backlog, each story is driven implement → verify-against-its-Gherkin → review to Definition of Done, and the sprint **loops until every story is Done** (`keel sprint review` is the fail-closed gate). Nothing half-built is presented as complete.
