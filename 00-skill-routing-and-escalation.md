...
Token-saving rule: prevent noisy raw command output from entering the harness context. Route through `keel run -- <command>` or rely on the hook's transparent rewrite before noisy output is produced.
...
- **Hook transparent rewrite**: the managed `PreToolUse` hook transparently rewrites supported shell commands via `toolInputOverride` into `keel run -- <command>`. Execution proceeds automatically with the wrapped command; no manual rerun is needed.
...
4. **Iterative loop**: ALIGN → RESEARCH → PLAN → IMPACT → IMPLEMENT → TEST → FIX → VERIFY → REVIEW → RECONCILE. For multi-piece or multi-step builds, run this as **Anvil** (`running-anvil` / `keel anvil`): compile a named bar into lock+prefix+gates, cast isolated workspaces, sieve with 0-LLM gates, stamp the winner, and bounded-loop only if gates still fail. MCP uses `compile` then `run --dry-run`; live `run`/`loop` stay on the CLI. Nothing half-built is presented as complete.
...
6. **Release ladder is fail-closed for non-trivial and release-facing work**: Smoke → Functional → Integration → UI → Load → Stress → Security. Mark not-applicable only with explicit, evidence-backed reasoning.
...
10. **Mandatory external research**: every fix, trace, or implementation starts with a live web lookup (WebSearch/WebFetch/context7; Stack Overflow or vendor docs as fitting) for that problem. Model memory is stale by cutoff; `recall`, memory, and host reads are a hypothesis, never proof. The requirement is **per problem**, not per session: a new problem needs new evidence, and reuse happens only through a fresh `research-cache` entry. The PreToolUse Iron Law gate defaults to **Verified** (fresh external research required) and the PostToolBatch/Stop research gate is per-problem. See [AGENTS.md](AGENTS.md) rule 11 and [30-execution-strategy.md](AGENTS/references/30-execution-strategy.md) § 1.
