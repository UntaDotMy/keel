...
4. **Iterative loop** — ALIGN → RESEARCH → PLAN → IMPLEMENT → TEST → FIX → VERIFY → REVIEW → RECONCILE. For multi-story or multi-step builds, run this as a **Scrum-style sprint** (`running-a-sprint`): the confirmed user stories become the sprint backlog, each story is driven implement → verify-against-its-Gherkin → review to Definition of Done, and the sprint **loops until every story is Done** (`claude-skills sprint review` is the fail-closed gate). Nothing half-built is presented as complete.
