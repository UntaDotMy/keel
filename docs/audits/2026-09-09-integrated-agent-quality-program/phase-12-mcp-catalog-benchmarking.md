...
2. **Wire Discovery Footprint Reduction**:
   ...
   - Token weight of `mcp.tools_list.catalog` dropped from 2,929 tokens down to **1,225 tokens** (**58.18% reduction**, saving **1,704 tokens** on every agent session initialization).
   - Framed payload remains well within the stdio frame ceiling: the default tiered response measures 3,335 bytes and the full profile 7,158 bytes, leaving more than 16KB of headroom under the 24KB ceiling.
...
4. **Fixed-Context Ledger and Budget Updates**:
   - Updated `ratified_budget("mcp.tools_list.catalog")` in `rust/crates/keel/src/utility/fixed_context.rs` to **1,348 tokens** (`ceil(1,225 * 1.10)`).
...
| Context ledger snapshot reports real eager/deferred counts | `ToolsListContextSnapshot` reports 37 total, 17 eager, 20 deferred, 1,225 catalog tokens. |
| Update ratified budget with 10% headroom | `ratified_budget("mcp.tools_list.catalog")` updated to 1,348 tokens in `fixed_context.rs`. |
...
- **Fixed Context Budget**: 5 passed, 0 failed (`mcp.tools_list.catalog` verified at 1,225 actual / 1,348 budget).
