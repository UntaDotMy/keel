...
It is for a new operator who already has `keel` installed and wants one satisfying end-to-end run without memorizing every workflow command first.
...
- `keel workflow route --request "..."` — recommends a preset for a broad request
- `keel workflow start --request "..." [--preset autopilot|debug|tdd|review|eco|parallel]` — opens a ledger entry
- `keel workflow status` / `workflow cockpit` / `workflow dashboard` / `workflow watch` — read the same ledger from different angles
- `keel workflow resume --id <entry-id>` — reopens an existing entry
- `keel workflow finish --id <entry-id> --proof "..."` — closes an entry with proof attached
...
```bash
keel workflow route --request "Compare the current repo, fix the biggest gaps, and carry the branch to closure"
```
...
```bash
keel workflow start --preset autopilot --request "Compare the current repo, fix the biggest gaps, and carry the branch to closure"
```
...
- `autopilot`
  - `keel workflow start --preset autopilot --request "Carry the current task to closure"`
...
- `debug`
  - `keel workflow start --preset debug --request "Trace the failing behavior, fix it, and prove it"`
...
- `tdd`
  - `keel workflow start --preset tdd --request "Write the proving test first, then implement the feature"`
  ...
  - If interrupted: use `workflow cockpit`, `workflow resume --id <entry-id>`, and `keel memory completion-gate check --id <entry-id> --proof "..."`.
- `review`
...
- `eco`
  - `keel workflow start --preset eco --request "Carry the small maintenance task to closure"`
...
- `parallel`
  - `keel workflow start --preset parallel --request "Coordinate the next multi-lane task"`
...
```bash
keel workflow finish --id <entry-id> --proof "cargo test --workspace passed; review pre-pr passed; hosted checks green"
```
...
```bash
keel workflow start --request "Carry the current task to closure"
keel workflow cockpit
