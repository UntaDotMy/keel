...
| `memory research-cache`, `maintenance`, `agent-registry`, `agent-packets`, `loop-guard`, `entity`, `graph`, `retrieve`, `status` (and the same set under `memoriesv2`) | Supported | Supported | Supported | Supported | Family commands persisting flat-JSON records under `<claude-home>/<group>/<family>/`, isolated per command group. `memory report|index|hook` and `consolidate` are still not implemented and return a "not implemented" message. |
