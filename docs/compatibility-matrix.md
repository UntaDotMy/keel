...
| Google Antigravity | Global plugin rule plus gateway skill | CamelCase JSON hook adapter | Direct global and plugin `mcp_config.json` entries | IDE and `agy` CLI use different global roots; Keel detects each and registers MCP directly so it does not depend on plugin enablement. The hook command requires `node` on PATH and doctor verifies it. |
...
| Secret answer redaction | Supported | Secret-shaped answer values redacted on refuse Display paths; prefer env names in `locked_brief`. Answers remain untrusted. No eval or shell interpolation. |
