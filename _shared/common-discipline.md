...
- **Package selection and maintainability (no reinventing the wheel).** Before implementing non-trivial functionality from scratch, research mature, battle-tested community packages:
  ...
  - Reject abandoned or unmaintained packages; never build fragile custom parsers, state engines, or crypto primitives when standard, audited libraries exist.
  - Cross-platform cleanliness: scripts and outputs must format cleanly across Windows PowerShell and Unix Bash without garbled escape sequences, literal `\n` tokens, or mismatched directory separators.
