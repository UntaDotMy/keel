...
evidence. `keel skill-lint` is the structural gate (does the skill
...
- Stop when the skill holds across the pressure variants and reads clean. Then run
  `keel skill-lint` for the structural gate (name match, description +
...
- **Name the rationalization, then answer it.** A "Red Flags" table that lists the
  ...
  than an abstract rule. See `using-keel/SKILL.md` for the pattern.
...
Two gates, in order. Structural: `keel skill-lint` must pass (the skill
